...
echo "Releasing Orbit Meeting SDK ${SDK_VERSION}"
...
# Now build and publish the Orbit Meeting SDK and its dependencies
echo "Building and publishing the Orbit Meeting SDK"
pushd ${THIS_DIR}/../
./gradlew clean
...
if [[ $MVN_HTTP == 0 ]]; then
    pushd ${MVN_REPO_PATH}
    git add -A .
    git commit -m "Orbit Meeting SDK + dependencies: ${SDK_VERSION}"
    popd
fi
