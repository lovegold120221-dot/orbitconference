...
cp /usr/share/orbit-meet-web-config/nginx-jaas.conf /etc/jitsi/meet/jaas
sed -i "s/jaas_magic_cookie/${VPAAS_COOKIE}/g" /etc/jitsi/meet/jaas/nginx-jaas.conf

cp /usr/share/orbit-meet-web-config/8x8.vc-config.js /etc/jitsi/meet/jaas/
...
ln -s /usr/share/orbit-meet-web-config/index-jaas.html /usr/share/orbit-meet/index-jaas.html

# let's create the daily key now
/usr/share/orbit-meet/scripts/update-asap-daily.sh

# let's add to cron daily the update of the asap key
if [ -d /etc/cron.daily ]; then
  ln -s /usr/share/orbit-meet/scripts/update-asap-daily.sh /etc/cron.daily/update-jaas-asap.sh
else
  echo "No /etc/cron.daily. Please add to your cron jobs to execute as root daily the script: /usr/share/orbit-meet/scripts/update-asap-daily.sh"
fi
