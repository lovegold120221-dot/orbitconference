...
DOMAIN=$2
if [ -z "$DOMAIN" ]; then
  DEB_CONF_RESULT=$(debconf-show orbit-meet-web-config | grep orbit-meet/jvb-hostname)
...
RELOAD_CMD+=" && /usr/share/orbit-meet/scripts/coturn-le-update.sh ${DOMAIN}"

ISSUE_FAILED_CODE=0
ISSUE_CERT_CMD="/opt/acmesh/.acme.sh/acme.sh -f --issue -d ${DOMAIN} -w /usr/share/orbit-meet --server letsencrypt"
...
if [ ${ISSUE_FAILED_CODE} -ne 0 ] ; then
    ...
    if [ ${ISSUE_FAILED_CODE} -eq 2 ]; then
        ...
        echo "/usr/share/orbit-meet/scripts/install-letsencrypt-cert.sh $EMAIL"
    fi
else
    eval "$INSTALL_CERT_CMD"
fi
