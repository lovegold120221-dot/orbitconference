...
source-package: compile deploy
	mkdir -p source_package/orbit-meet/css && \
	cp -r *.js *.html resources/*.txt fonts images libs static sounds LICENSE lang source_package/orbit-meet && \
	cp css/all.css source_package/orbit-meet/css && \
	(cd source_package ; tar cjf ../orbit-meet.tar.bz2 orbit-meet) && \
